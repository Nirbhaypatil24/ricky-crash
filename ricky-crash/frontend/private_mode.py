...
class PrivateModeWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setup_ui()
        print("✅ Private Mode Widget Initialized")

    def setup_ui(self):
        layout = QVBoxLayout(self)  # Attach layout to self immediately
        ...
        self.fare_amount.setStyleSheet(f"color: {TEXT_MAIN}; font-size: 72px; font-weight: bold;")
        ...
        self.location_frame.setStyleSheet(f"background-color: {CARD_BG}; border: 1px solid #333; border-radius: 15px;")
        location_layout = QVBoxLayout(self.location_frame)
        
        loc_title = QLabel("CURRENT LOCATION")
        loc_title.setStyleSheet(f"color: {TEXT_SUB}; font-size: 12px; font-weight: bold;")
        loc_title.setAlignment(Qt.AlignCenter)
        
        self.location_lbl = QLabel("Locating...")
        self.location_lbl.setStyleSheet(f"color: {TEXT_MAIN}; font-size: 20px; font-weight: 900;")
    ...
    def create_stat_card(self, title, initial_value):
        frame = QFrame()
        frame.setStyleSheet(f"background-color: {CARD_BG}; border-radius: 15px; border: 1px solid #333;")
        vbox = QVBoxLayout(frame)
        
        lbl_value = QLabel(initial_value)
        lbl_value.setStyleSheet(f"color: {TEXT_MAIN}; font-size: 32px; font-weight: bold;")
