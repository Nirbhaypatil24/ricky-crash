...
Updated: Includes Live Graph Integration
"""
...
class SOSStatusWidget(QFrame):
    ...
    def _flash_tick(self):
        ...
        if self.flash_state:
            ...
            self.main_lbl.setStyleSheet(f"color: {THEME_DANGER};")

    def update_status(self, status):
        if "CRASH" in status or "ACTIVE" in status:
            self.flash_timer.start(200 if "CRASH" in status else 500)
...
class RickyUI(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.for_hire_widget = self.create_placeholder("🚕 FOR HIRE", "Ready") # Shows Graph
    ...
    def create_placeholder(self, t, s):
        ...
        if "HIRE" in t:
            ...
            l.addSpacing(30)
            self.sensor_graph = SensorGraphWidget() # Create Graph
    ...
    @pyqtSlot(float)
    def update_graph_data(self, g_force):
        """Receives live G-Force data from main.py and updates the graph"""
        if hasattr(self, 'sensor_graph'):
            self.sensor_graph.update_value(g_force)
    ...
    def update_sos_status(self, s):
        ...
        is_emergency = "ACTIVATED" in s or "CRASH" in s
        if is_emergency:
            self.header.hide()
